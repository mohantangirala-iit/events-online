...
type RestOf<T extends IConference | NewConference> = Omit<T, 'startDate' | 'endDate'> & {
...
export class ConferenceService {
  ...
  protected convertDateFromClient<T extends IConference | NewConference | PartialUpdateConference>(conference: T): RestOf<T> {
    return {
      ...conference,
      startDate: conference.startDate?.toJSON() ?? null,
      endDate: conference.endDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restConference: RestConference): IConference {
    return {
      ...restConference,
      startDate: restConference.startDate ? dayjs(restConference.startDate) : undefined,
      endDate: restConference.endDate ? dayjs(restConference.endDate) : undefined,
    };
  }
