...
describe('ApplicationUser e2e test', () => {
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/users',
      body: {"login":"7uDsN.","firstName":"Ulises","lastName":"Wehner","email":"Adelia_Breitenberg-Barton94@hotmail.com","imageUrl":"how vaguely flatter","langKey":"meh despit"},
    }).then(({ body }) => {
      user = body;
    });
  });
  ...
  beforeEach(() => {
    ...
    cy.intercept('GET', '/api/person-types', {
      statusCode: 200,
      body: [],
    });

    cy.intercept('GET', '/api/conferences', {
      statusCode: 200,
      body: [],
    });
  ...
  describe('new ApplicationUser page', () => {
    ...
    it.skip('should create an instance of ApplicationUser', () => {
      cy.get(`[data-cy="phoneNumber"]`).type('modest monumental besides');
      cy.get(`[data-cy="phoneNumber"]`).should('have.value', 'modest monumental besides');
