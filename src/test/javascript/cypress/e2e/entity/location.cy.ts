...
describe('Location e2e test', () => {
  ...
  describe('new Location page', () => {
    ...
    it('should create an instance of Location', () => {
      cy.get(`[data-cy="streetAddress"]`).type('athletic');
      cy.get(`[data-cy="streetAddress"]`).should('have.value', 'athletic');

      cy.get(`[data-cy="postalCode"]`).type('fabricate');
      cy.get(`[data-cy="postalCode"]`).should('have.value', 'fabricate');

      cy.get(`[data-cy="city"]`).type('North Ivory');
      cy.get(`[data-cy="city"]`).should('have.value', 'North Ivory');

      cy.get(`[data-cy="stateProvince"]`).type('enormously now aside');
      cy.get(`[data-cy="stateProvince"]`).should('have.value', 'enormously now aside');

      cy.get(`[data-cy="countryName"]`).type('woot');
      cy.get(`[data-cy="countryName"]`).should('have.value', 'woot');

      cy.get(`[data-cy="region"]`).select('APAC');
